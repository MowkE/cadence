...
function createCloud({ config = {}, tokenFile, openExternal, appVersion = '', emulator = null, log = console }) {
    ...
    const bucket = String(config.storageBucket || (projectId ? `${projectId}.appspot.com` : '')).trim();
    ...
    // Profile
    // ------------------------------------------------------------------
    async function claimHandle(base, uid) {
        const root = handleFrom(base);
        for (let i = 0; i < 12; i++) {
            const candidate = i === 0 ? root : `${root.slice(0, 16)}${Math.floor(100 + Math.random() * 900)}`;
            try {
    ...
    async function ensureProfile(idp) {
        ...
        const fallback = (idp.email || '').split('@')[0] || 'cadence';
        const displayName = cleanText(idp.displayName || fallback, 40) || 'Cadence listener';
        const handle = await claimHandle(idp.displayName || fallback, uid);
        const now = new Date();
    ...
    async function updateProfile({ displayName, handle } = {}) {
        ...
        if (displayName !== undefined) {
            ...
            patch.displayName = name; mask.push('displayName');
        }
        let oldHandle = null;
        if (handle !== undefined) {
            const h = String(handle || '').toLowerCase().trim().replace(/^@/, '');
            if (!HANDLE_RE.test(h)) return { success: false, error: 'Handles are 3–20 letters, numbers or underscores' };
            if (h !== profile.handle) {
                try {
                    await fsSet(`handles/${h}`, { uid: tokens.uid }, { mustNotExist: true });
                } catch (err) {
                    if (err.status === 409 || err.code === 'ALREADY_EXISTS') return { success: false, error: `@${h} is taken` };
                    return fail(`Could not change handle: ${err.message}`, err);
                }
                oldHandle = profile.handle;
                patch.handle = h; mask.push('handle');
            }
        }
        if (!mask.length) return { success: true, status: status() };
        patch.updatedAt = new Date(); mask.push('updatedAt');
        try {
            profile = await fsSet(`users/${tokens.uid}`, patch, { mask });
            if (oldHandle) await fsDelete(`handles/${oldHandle}`);
            lastError = null;
            emit();
            return { success: true, status: status() };
        } catch (err) {
    ...
    return {
        configured, status, onChange, restore,
