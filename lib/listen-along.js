...
// control / playTrack, routed to the Web API or the local Spotify app
function createListenAlong({ getRelayBase, player, onStatus, log = console }) {
    ...
    async function startHost() {
        ...
        hostName = null;
        try {
            const profile = player && player.isConnected() ? await player.getProfile() : null;
    ...
    async function mirrorPlayback() {
        ...
        // A host reading the Spotify app on Windows has no track links to share
        if (state.track && state.is_playing && !state.track.uri) {
            mirrorNote = "The host's Spotify doesn't share track links, so play-along isn't available — following the lyrics only";
            emit();
            return;
        }
        mirrorBusy = true;
