...
function createSpotifyMac(log) {
    ...
    async function read() {
        try {
            ...
            if (err.permission) return { success: false, error: err.message, needs_permission: true, message: PERMISSION_HINT, setup: { kind: 'automation', browser: 'Spotify' } };
    ...
    // Cross-service listen-along. The Spotify app can play a known Spotify uri,
    // but it can't search — so when the host is on another service (no uri to
    // share) there is nothing we can resolve here. Connecting the Web API keys
    // is what unlocks search, hence the hint.
    function playBySearch({ uri, positionMs } = {}) {
        if (/^spotify:(track|episode):[A-Za-z0-9]+$/.test(String(uri || ''))) {
            return playTrack(uri, positionMs);
        }
        return Promise.resolve({
            success: false, command: 'play',
            error: 'Matching the host’s song needs your Spotify keys (gear → 🔑) — following the lyrics',
            reason: 'NO_SEARCH'
        });
    }

    return { read, control, playTrack, playBySearch, canPlayTrack: true };
}
...
// Cross-service listen-along (search-and-play). Navigating the tab to a search
// URL loads real results; the first song row exposes a `watch?v=<id>` link, and
// opening that watch URL autoplays the song. No regex in the injected JS keeps
// AppleScript/base64 quoting simple.
const ytmNavJs = url => `(() => { try { location.href = ${JSON.stringify(url)}; return 'ok'; } catch (e) { return 'err'; } })()`;

// Reads the first song result's video id off the rendered search page. Given
// the wanted title it prefers a row that mentions it and skips live/karaoke/etc.
// variants (unless the host's own title has that word), so we land on the studio
// cut rather than whatever YouTube happened to rank first. Returns 'not-search'
// before the page has loaded and 'none' when there are no results yet.
const ytmExtractJs = (wantTitle) => `(() => {
  if (location.pathname.indexOf('/search') !== 0) return 'not-search';
  const want = ${JSON.stringify(String(wantTitle || '').toLowerCase())};
  const bad = ['live','karaoke','cover','instrumental','sped up','slowed','remix','reverb'];
  const idFrom = (el) => {
    if (!el) return '';
    const href = el.getAttribute('href') || '';
    const i = href.indexOf('watch?v=');
    if (i < 0) return '';
    let id = href.slice(i + 8);
    const amp = id.indexOf('&'); if (amp >= 0) id = id.slice(0, amp);
    return id;
  };
  const rows = Array.prototype.map.call(document.querySelectorAll('ytmusic-responsive-list-item-renderer'),
    r => ({ id: idFrom(r.querySelector('a[href*="watch?v="]')), text: (r.textContent || '').toLowerCase() }))
    .filter(r => r.id);
  if (!rows.length) return idFrom(document.querySelector('a[href*="watch?v="]')) || 'none';
  const wantHasBad = bad.some(b => want.indexOf(b) >= 0);
  const clean = rows.find(r => (!want || r.text.indexOf(want) >= 0) && (wantHasBad || !bad.some(b => r.text.indexOf(b) >= 0)));
  return (clean || rows[0]).id;
})()`;
...
function createYtmMac(log) {
    ...
    async function read() {
        ...
        for (const browser of browsersInOrder()) {
            try {
                ...
                if (err.permission) return { success: false, error: err.message, needs_permission: true, app: 'ytmusic', message: `Allow Cadence to control ${browser.name}: System Settings → Privacy & Security → Automation`, setup: { kind: 'automation', browser: browser.name } };
        ...
        if (blocked) {
            return { success: false, error: 'JavaScript from Apple Events is off', needs_permission: true, app: 'ytmusic', message: jsBlockedHint(blocked), setup: { kind: 'ytmusic-js', browser: blocked.name, browserKind: blocked.kind } };
        }
        lastBrowser = null;
    ...
    // Run one JS snippet in the YTM tab, trying the known browsers in order.
    // { out } is the tab's reply, { out: null } means no tab was found, and
    // { err, browser } means the browser blocked JS-from-Apple-Events / denied automation.
    async function evalJs(js) {
        for (const browser of browsersInOrder()) {
            try {
                ...
                if (err.jsBlocked || err.permission) return { err, browser };
                // timeouts / transient errors: try the next browser
            }
        }
        return { out: null };
    }
    ...
    // Cross-service listen-along: resolve the host's song by title + artist on
    // YouTube Music and play it. It is multi-step and inherently flaky (search
    // has to render, then the watch page has to load), so it drives itself in
    // the background and never throws — navigate to the search page, read the
    // first song's video id, open its watch URL (which autoplays), then seek to
    // where the host is. A newer call cancels an in-flight one.
    async function playBySearch({ name, artist, positionMs } = {}) {
        const query = [name, artist].map(s => String(s || '').trim()).filter(Boolean).join(' ').slice(0, 200);
        if (!query) return { success: false, command: 'play', error: 'The host is not sharing a song to match', reason: 'NO_QUERY' };

        const gen = ++searchGen;
        const startedAt = Date.now();
        const basePos = Math.max(0, Number(positionMs) || 0);

        const nav = await evalJs(ytmNavJs('https://music.youtube.com/search?q=' + encodeURIComponent(query)));
        if (nav.err) {
            if (nav.err.jsBlocked) return { success: false, command: 'play', error: jsBlockedHint(nav.browser), needs_permission: true };
            return { success: false, command: 'play', error: String(nav.err.message || '').slice(0, 160) || 'Could not reach YouTube Music', needs_permission: Boolean(nav.err.permission) };
        }
        if (nav.out === null) return { success: false, command: 'play', error: 'Open YouTube Music in your browser first', reason: 'NOT_RUNNING' };

        // Steps 2-4 run detached; report "searching" straight away.
        (async () => {
            let id = null;
            for (let i = 0; i < 8 && gen === searchGen; i++) {
                await delay(700);
                if (gen !== searchGen) return;
                const r = await evalJs(ytmExtractJs(name));
                const out = r.out;
                if (out && out !== 'none' && out !== 'not-search' && /^[\w-]{6,}$/.test(out)) { id = out; break; }
            }
            if (!id || gen !== searchGen) return;
            await evalJs(ytmNavJs('https://music.youtube.com/watch?v=' + id));
            // Catch up to the host once the video is seekable
            for (let i = 0; i < 6 && gen === searchGen; i++) {
                await delay(700);
                if (gen !== searchGen) return;
                const r = await evalJs(YTM_CONTROL_JS.seek(basePos + (Date.now() - startedAt)));
                if (r.out === 'ok') break;
            }
        })().catch(() => {});

        return { success: true, command: 'play', searching: true };
    }

    return { read, control, playTrack, playBySearch, canPlayTrack: false, describe: () => lastBrowser ? `YouTube Music in ${lastBrowser.name}` : 'YouTube Music in your browser' };
}
...
function createMacBackend(log, getApp) {
    ...
    return {
        ...
        playBySearch: (opts) => backendFor().playBySearch(opts),
...
function createWindowsBackend(log, getApp) {
    ...
    return {
        ...
        // The system media session can't start a specific song (no uri, no search)
        playBySearch: () => Promise.resolve({ success: false, command: 'play', error: 'Playing along on Windows needs Spotify API keys (gear → 🔑) — following the lyrics only', reason: 'UNSUPPORTED' }),
        canPlayTrack: () => false,
        describe: () => {
            const app = getApp();
...
function createLocalPlayer({ log = console, app = 'auto' } = {}) {
    ...
    return {
        ...
        playBySearch: (opts) => backend ? backend.playBySearch(opts) : Promise.resolve({ ...unsupported, command: 'play' }),
        getProfile,
