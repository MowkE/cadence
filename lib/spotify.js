...
// ---------------------------------------------------------------------------
// Cross-service search matching (listen-along). A host on any service shares
// title + artist + duration; a guest on Spotify resolves the same song among
// the Web API search results. Kept as pure, exported functions so the matching
// can be unit-tested without a network or credentials.
// ---------------------------------------------------------------------------
function normalizeMatch(s) {
    return String(s || '').toLowerCase()
        .replace(/\(.*?\)|\[.*?\]/g, ' ')     // (feat. …), [Remastered]
        .replace(/\bfeat\.?\b.*$/, ' ')       // feat. …, ft. …
        .replace(/[^a-z0-9]+/g, ' ')
        .trim();
}

function tokenOverlap(a, b) {
    const at = new Set(normalizeMatch(a).split(' ').filter(Boolean));
    const bt = new Set(normalizeMatch(b).split(' ').filter(Boolean));
...
function titleScore(candName, wantName) {
    const c = normalizeMatch(candName), w = normalizeMatch(wantName);
    if (!w || !c) return 0;
    if (c === w) return 100;
    if (c.includes(w) || w.includes(c)) return 65;
    return 45 * tokenOverlap(candName, wantName);
}

function artistScore(cand, wantArtist) {
    const raw = Array.isArray(cand.artists) ? cand.artists.join(' ') : (cand.artist || '');
    const c = normalizeMatch(raw), w = normalizeMatch(wantArtist);
    if (!w || !c) return 0;
    if (c === w) return 45;
    if (c.includes(w) || w.includes(c)) return 30;
    return 35 * tokenOverlap(raw, wantArtist);
}

function scoreCandidate(cand, want) {
    let score = titleScore(cand.name, want.name) + artistScore(cand, want.artist);
    if (want.durationMs && cand.duration_ms) {
        const diffSec = Math.abs(Number(cand.duration_ms) - Number(want.durationMs)) / 1000;
        score += Math.max(0, 15 - diffSec);   // 15 for a perfect match, −1 per second off
    }
    return score;
}

// Best of the candidate tracks, or null when nothing is close enough to risk
// playing. Each candidate is { uri, name, artists|artist, duration_ms }.
function pickBestMatch(candidates, want = {}) {
    const list = Array.isArray(candidates) ? candidates.filter(c => c && c.uri && c.name) : [];
    ...
    if (!best) return null;
    // Never play a wild mismatch: the winner must share something with the title
    if (titleScore(best.name, want.name) < 15) return null;
    return best;
}
...
function createSpotifyClient({ tokenFile, getCredentials, openExternal, legacyCacheFiles = [], log = console }) {
    ...
    // Cross-service listen-along: play the host's song on this Spotify account.
    // With a Spotify uri already in hand we just play it; otherwise we search by
    // title + artist and play the closest match (so a YouTube Music host can be
    // mirrored on a Spotify guest).
    async function playBySearch({ name, artist, durationMs, uri, positionMs } = {}) {
        if (uri && /^spotify:(track|episode):[A-Za-z0-9]+$/.test(String(uri))) {
            return playTrack(uri, positionMs);
        }
        const query = [name, artist].map(s => String(s || '').trim()).filter(Boolean).join(' ').slice(0, 250);
        if (!query) {
            return { success: false, command: 'play', error: 'The host is not sharing a song to match', reason: 'NO_QUERY' };
        }
        const res = await request('GET', '/search', { query: { q: query, type: 'track', limit: 5 } });
        if (!res.success) {
            return { success: false, command: 'play', error: res.error, error_code: res.status || null, reason: res.reason, needs_auth: res.needs_auth };
        }
        const items = (res.data && res.data.tracks && res.data.tracks.items) || [];
        ...
        const best = pickBestMatch(candidates, { name, artist, durationMs });
        if (!best) {
            return { success: false, command: 'play', error: `Couldn't find "${name || 'that song'}" on Spotify — following the lyrics`, reason: 'NOT_FOUND' };
        }
        return playTrack(best.uri, positionMs);
    }
...
module.exports = { createSpotifyClient, REDIRECT_URI, SCOPES, pickBestMatch, scoreCandidate, normalizeMatch };
